...
class Settings:
    ...
    # locations
    log_location = localize_path("YoutubePy", "logs")
    OS_ENV = ("windows" if platform == "win32"
        else "osx" if platform == "darwin"
        else "linux")

    specific_chromedriver = "chromedriver_{}".format(OS_ENV)
    chromedriver_location = localize_path("YoutubePy", "assets", specific_chromedriver)
    if (not chromedriver_location or not path_exists(chromedriver_location)):
        chromedriver_location = localize_path("YoutubePy", "assets", "chromedriver")
    ...
    # set a logger cache outside the YoutubePy object to avoid
    ...
    WORKSPACE = {"name": "YoutubePy", "path": environmental_variables.get("HOME")}

    DATABASE_LOCATION = localize_path("YoutubePy", "db", "youtubepy.db")
...
# state of instantiation of YoutubePy
